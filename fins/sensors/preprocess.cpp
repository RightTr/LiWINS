...
void Preprocess::process(const Pcl2MsgConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  switch (time_unit)
  {
    case SEC:
...
void Preprocess::avia_handler(const LivoxCustomMsgConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::velodyne_handler(const Pcl2MsgConstPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
...
void Preprocess::airy_handler(const Pcl2MsgConstPtr& msg) {
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  pl_corn.reserve(plsize);
  pl_surf.reserve(plsize);
  double timestamp_beg = pl_orig.points[0].timestamp;
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      added_pt.curvature = (pl_orig.points[i].timestamp - timestamp_beg) * time_unit_scale;
      if(pl_orig.points[i].ring < N_SCANS)
      {
        pl_buff[pl_orig.points[i].ring].push_back(added_pt);
      }
...
void Preprocess::sim_handler(const Pcl2MsgConstPtr &msg) {
    pl_surf.clear();
    pl_full.clear();
